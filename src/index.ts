...
function safeLog(message: unknown, data?: unknown): void {
  ...
  if (message instanceof Error) {
    ...
    try {
      ...
      formattedMessage += `[Unstringifiable Object: ${Object.prototype.toString.call(
        message
      )}]`
    }
  } else {
...
class PerplexityMCPServer {
      ...
      switch (recoveryLevel) {
        ...
        default:
          ...
          await new Promise((resolve) =>
            setTimeout(resolve, CONFIG.RECOVERY_WAIT_TIME)
          )
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        ...
        if (isNavigationError) {
          ...
          const navWaitTime = Math.min(
            8000 * consecutiveNavigationErrors,
            40000
          )
          safeLog(
            `Waiting ${navWaitTime / 1000} seconds after navigation error...`
          )
        ...
        if (isConnectionError || isProtocolError) {
          ...
          safeLog(
            ...
            )} seconds after connection error...`
          )
          await new Promise((resolve) =>
            setTimeout(resolve, connectionWaitTime)
          )
          continue
        }
        ...
        safeLog(
          `Retrying in ${Math.round(delay / 1000)} seconds (base: ${Math.round(
            baseDelay / 1000
          )}s, jitter: ${Math.round(jitter / 1000)}s)...`
        )
        ...
        try {
          ...
          safeLog(
            `Navigation failed, waiting ${Math.round(
              navFailWaitTime / 1000
          ...
          if (i > 1) {
            logError(
              'Multiple navigation failures, attempting full recovery...'
            )
            await this.recoveryProcedure()
          }
        }
  ...
  private async waitForCompleteAnswer(page: Page): Promise<string> {
    ...
    const answerPromise = page.evaluate(async () => {
      ...
      return (
        lastAnswer ||
        'No answer content found. The website may be experiencing issues.'
      )
    })
      ...
      try {
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
          try {
            ...
            logError(
              `Attempt ${attempt + 1} to get partial answer failed:`,
              evalError
            )
  ...
  private async performSearch(query: string): Promise<string> {
    ...
    try {
      ...
      if (!this.browser || !this.page || (this.page && this.page.isClosed())) {
        safeLog(
          'Browser/page not initialized or page closed, initializing now...'
        )
      ...
      return await this.retryOperation(async () => {
        safeLog(
          `Navigating to Perplexity for query: "${query.substring(0, 30)}${
            query.length > 30 ? '...' : ''
          }"`
        )
        await this.navigateToPerplexity()
        ...
        if (!this.page || this.page.mainFrame().isDetached()) {
          logError(
            'Main frame is detached, will retry with new browser instance'
          )
        ...
        if (!selectorFound) {
          ...
          await this.page.screenshot({
            path: 'debug_prose_not_found.png',
            fullPage: true,
          })
          ...
          if (pageText && pageText.length > 200) {
            safeLog(
              'Found text content on page, attempting to extract answer...'
            )
      ...
      return `The search operation could not be completed. Error: ${
        error instanceof Error ? error.message : 'Unknown error'
      }. Please try again later with a more specific query.`
    } finally {
  ...
  private async handleChatPerplexity(args: {
    message: string
    chat_id?: string
  }): Promise<string> {
  ...
  private async handleGetDocumentation(args: {
    query: string
    context?: string
  }): Promise<string> {
...
10. Related tools/libraries that work well with it`
  ...
  private async handleFindApis(args: {
    requirement: string
    context?: string
  }): Promise<string> {
      ...
      tools: [
        {
          ...
          description:
            'Maintains ongoing conversations with Perplexity AI using a persistent chat history. Starts new chats or continues existing ones with full context. Returns a stringified JSON object containing chat_id and response.',
          ...
          examples: [
            ...
            },
            {
              ...
              output: {
                chat_id: 'existing-chat-id',
                response:
                  'Classical computers use bits that are either 0 or 1, while quantum...',
              },
            },
          ],
          ...
          outputSchema: {
            ...
            properties: {
              response: {
                type: 'string',
                description:
                  'The raw text response from Perplexity containing documentation and examples.',
              },
            },
          },
          examples: [
          ...
          inputSchema: {
            ...
            properties: {
              ...
              context: {
                ...
                examples: [
                  'prefer free tier options',
                  'must support Python SDK',
                ],
              },
            },
          ...
          examples: [
            {
              ...
              output: {
                ...
                comparison:
                  'PayPal has broader global support while Square offers more business tools...',
              },
            },
            {
              ...
              output: {
                apis: [
                  ...
                  },
                  {
                    ...
                    pricing:
                      'Free tier available, then $0.50 per 1000 requests',
                    documentation: 'https://docs.mapbox.com/api',
                  },
                ],
                comparison:
                  'Google offers higher accuracy but Mapbox is more cost-effective...',
              },
            },
          ],
          ...
          examples: [
            {
              ...
              output: {
                deprecated_items: [
                  {
                    ...
                    reason:
                      'Legacy lifecycle method, unsafe for async rendering',
                ...
                migration_guide:
                  '1. Move initialization code to constructor\n2. For side effects, use componentDidMount\n3. Consider using useEffect for functional components',
                compatibility_notes:
                  'This change is required for React 17+ and concurrent mode features',
              },
            },
            {
              ...
              output: {
                ...
                migration_guide:
                  '1. Add parentheses around print arguments\n2. Run 2to3 tool for bulk conversion\n3. Test thoroughly for behavioral differences',
                compatibility_notes:
                  'Python 2 reached end-of-life in 2020, upgrade is strongly recommended',
              },
            },
          ],
          related_tools: ['get_documentation', 'search'],
    ...
    this.server.setRequestHandler(
      ...
      async (request: any) => {
        ...
        const requestTimeout = setTimeout(() => {
          logError(
            'MCP request is taking too long, this might lead to a timeout'
          )
        ...
        try {
          ...
          if (toolName === 'chat_perplexity') {
            const chatId =
              request.params.arguments.chat_id || crypto.randomUUID()
            return {
              content: [
                {
                  type: 'text',
                  text: JSON.stringify(
                    { chat_id: chatId, response: responseContent },
                    null,
                    2
                  ),
                },
